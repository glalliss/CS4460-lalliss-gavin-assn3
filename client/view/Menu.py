...
class Menu(tk.Frame):
    ...
    def __init__(self, master: tk.Frame, title: str, root: tk.Tk, *args, **kwargs) -> None:
        ...
        # persistent option when set to return to the previous menu
        self.__back_option: Menu.MenuOption | None = None
        # list of menu options for the user to select from
        self.__options: list[Menu.MenuOption] = []
        # index of the currently selected option
        self.__selected: int = 0
        # future list of text entries for getting input
        self.__input_texts: list[tk.Entry] = []
        # future window for getting input
        self.__input_text_window: tk.Toplevel | None = None
        # future callback to call after receiving input
        self.__input_text_callback: Callable | None = None
        # string that gets displayed in the read only display
        self.__text_display: str = ""
        # optional header that separates the read only display from the selectable options
        self.__options_header: str = "\n****Options****\n"
        # optional callback that gets called when returning to this menu from a submenu
        self.__on_return_to_menu_callback: Callable | None = None
    ...
    def edit_option(self, index: int, name: str | None = None, callback: Callable | None = None, *args,
                    **kwargs) -> None:
        """
        @param index: int
        ...
        Edits the option at position index. If a parameter is not passed, it defaults to None and that specific
            field is not updated in the option.
        """
    ...
    def get_input(self, count: int, window_title: str, callback: Callable, *prompts) -> None:
        ...
        @param *prompts
            A list of prompts to display. One prompt for each desired input.
            The number of prompts should be the same as count
    ...
    def __set_back_option(self, callback: Callable, *args, **kwargs):
        if str(callback) == "<bound method Menu.__switch_back of <client.view.Login.Login object .!login>>":
            self.__back_option = Menu.MenuOption("\nLogout", callback, *args, **kwargs)
        else:
